...
class LSTMBoxClassifier(torch.nn.Module):
    def __init__(self, input_size: int, extraction_label2id: Dict[str, Dict[str, int]], n_tags: int, hidden_size: int, num_layers: int, dropout: float, bidirectional: bool):
        super().__init__()
        ...
        for key, labels in extraction_label2id.items():
            ...
            self.classifiers[key] = LSTMPredictionHead(
...
class LSTMPredictionHead(torch.nn.Module):
    def __init__(self, input_size: int, n_tags: int, num_labels: int, hidden_size: int, num_layers: int, dropout: float, bidirectional: bool):
        ...
        self.gru = torch.nn.LSTM(input_size=input_size+n_tags, hidden_size=hidden_size,
